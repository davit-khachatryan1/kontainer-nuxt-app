<template>
	<section class="signup-account" :class="{ 'standalone': type === 'standalone' }">
		<template v-if="type === 'standalone'">
			<button class="form-close" :key="'close'" @click.prevent="close">{{ langString('_close') }}</button>
		</template>
					...
					<div class="signup-account__info" v-if="!isFreeKontainer()">
						<div class="row">
							<div class="signup-account__info__specification">
								...
								<div class="services" v-html="kontainerPackage.services"
									v-if="kontainerPackage.services" />
								<div class="services" v-if="kontainerPackage.features">
									<ul>
										<li v-for="feature in kontainerPackage.features" v-html="feature.name" />
									</ul>
								</div>
							</div>
							<div class="signup-account__info__pricing">
								...
								<span
									:class="{ 'price': true, 'extras': store.extraStorage[kontainerPackage.slug] || store.extraUsers[kontainerPackage.slug] }"
						...
						<div class="row extras" v-if="store.extraStorage[kontainerPackage.slug]">
							<div class="signup-account__info__specification">
								<h3>{{ langString('_extra_storage') }}:
									{{ store.extraStorage[kontainerPackage.slug].storage }}
									{{ store.extraStorage[kontainerPackage.slug].storage_unit }}</h3>
							</div>
							<div class="signup-account__info__pricing">
								<span class="currency" v-html="store.extraStorage[kontainerPackage.slug].currency" />
								<span class="price extras" v-html="store.extraStorage[kontainerPackage.slug].price" />
								<span class="monthly" v-html="kontainerPackage.per_month" />
							</div>
						</div>
						<div class="row extras" v-if="store.extraUsers[kontainerPackage.slug]">
							<div class="signup-account__info__specification">
								<h3>{{ langString('_extra_users') }}:
									{{ store.extraUsers[kontainerPackage.slug].users }}</h3>
							</div>
							<div class="signup-account__info__pricing">
								<span class="currency" v-html="store.extraUsers[kontainerPackage.slug].currency" />
								<span class="price extras" v-html="store.extraUsers[kontainerPackage.slug].price" />
								<span class="monthly" v-html="kontainerPackage.per_month" />
							</div>
						</div>
						<div class="row extras" v-if="store.extraSKUs[kontainerPackage.slug]">
							<div class="signup-account__info__specification">
								<h3>{{ langString('_extra_skus') }}:
									{{ store.extraSKUs[kontainerPackage.slug].users }}</h3>
							</div>
							<div class="signup-account__info__pricing">
								<span class="currency" v-html="store.extraSKUs[kontainerPackage.slug].currency" />
								<span class="price extras" v-html="store.extraSKUs[kontainerPackage.slug].price" />
								<span class="monthly" v-html="kontainerPackage.per_month" />
							</div>
						</div>
						<div class="row extras totals"
							v-if="store.extraStorage[kontainerPackage.slug] || store.extraUsers[kontainerPackage.slug] || store.extraSKUs[kontainerPackage.slug]">
							<div class="signup-account__info__specification">
										...
										<div class="columns__item columns__item--column">
											<div v-for="(paragraph, index) in extraTexts" :key="index" class="quote">
												<div class="quote__icon">
													<smart-image :class="'quoteicon'" :image="paragraph.icon" nocrop />
												</div>
...
export default {
	...
	props: {
		data: { type: Object },
		type: { type: String },
		btnSubmitText: { type: String },
	},
	computed: {
		subtitle() {
			if (this.kontainerPackage) {
				return this.langString('_signup_subtitle').replace(
					'{{name}}',
					this.kontainerPackage.title,
				);
			} else if (this.freeSubtitle) {
		...
		freeSubtitle() {
			return this.langString('_signup_subtitle_free');
		},
		extraTexts() {
	...
	methods: {
		...
		isFreeKontainer() {
			if (this.kontainerPackage && this.kontainerPackage.price === '0') {
				return true;
			} else if (this.kontainerPackage && this.kontainerPackage.price !== '0') {
				return false;
			}
		...
		submit(validate) {
			...
			validate().then((result) => {
				...
				if (!this.isFreeKontainer()) {
					const { price, currency, package_id: type } = this.kontainerPackage;
		...
		checkDomainAvailability() {
			...
			} else if (this.registration.company) {
				domain = this.registration.company.toLowerCase();
			} else {
			...
			$fetch(`${config.public.appUrl}/api/signup/availability`, {
				...
				.catch((error) => {
					...
					this.registration.host = urlSuggestion;

					if (urlSuggestion) {
						this.hostError = `${domain} ${this.langString(
							'_is_taken_how_about_this?',
						)}`;
					}
				});
		},
		getKontainerPackage(slug) {
			const store = useStore();
		...
		calculateTotals(slug) {
			const store = useStore();
			const kontainerPrice = parseInt(this.kontainerPackage.price, 10);
			const storagePrice = store.extraStorage[slug] ? parseInt(store.extraStorage[slug].price, 10) : 0;
			const usersPrice = store.extraUsers[slug] ? parseInt(store.extraUsers[slug].price, 10) : 0;
	...
	mounted() {
		...
		// this.$validator.localize(store.locale, this.dictionary);

		if (store.locale === 'da') {
			this.setCountryDenmark();
		}
	},
	created() {
